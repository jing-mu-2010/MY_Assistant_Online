...
lv_disp_t *bsp_display_start_with_config(const bsp_display_cfg_t *cfg)
{
    ...
    /* BOX1: touch init may fail on custom I2C bus — make it non-fatal */
    tp = NULL;
    esp_err_t tp_err = bsp_touch_new(NULL, &tp);
    if (tp_err == ESP_OK && tp != NULL) {
        const lvgl_port_touch_cfg_t touch_cfg = {
            .disp = disp,
            .handle = tp,
        };
        ...
        ESP_LOGW(TAG, "Touch init skipped (err=0x%x), using button-only input", tp_err);
        disp_indev = NULL;
    }
