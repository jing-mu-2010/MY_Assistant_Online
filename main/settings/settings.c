...
err:
    ...
    strlcpy(g_sys_param.ssid, "Not configured", sizeof(g_sys_param.ssid));
    strlcpy(g_sys_param.password, "", sizeof(g_sys_param.password));
    strlcpy(g_sys_param.key, "sk-xxxxxxxx", sizeof(g_sys_param.key));
    ESP_LOGW(TAG, "Using fallback settings because NVS configuration is incomplete");
    return ESP_OK;
}
...
esp_err_t settings_save_parameter_to_nvs(const char *ssid, const char *password, const char *key)
{
    ...
    ret = nvs_set_str(my_handle, "ssid", ssid ? ssid : "");
    ESP_GOTO_ON_FALSE(ESP_OK == ret, ret, err, TAG, "nvs set ssid failed (0x%x)", ret);

    ret = nvs_set_str(my_handle, "password", password ? password : "");
    ESP_GOTO_ON_FALSE(ESP_OK == ret, ret, err, TAG, "nvs set password failed (0x%x)", ret);

    ret = nvs_set_str(my_handle, "ChatGPT_key", key ? key : "");
    ESP_GOTO_ON_FALSE(ESP_OK == ret, ret, err, TAG, "nvs set key failed (0x%x)", ret);

    ret = nvs_commit(my_handle);
    ESP_GOTO_ON_FALSE(ESP_OK == ret, ret, err, TAG, "nvs commit failed (0x%x)", ret);

    strlcpy(g_sys_param.ssid, ssid ? ssid : "", sizeof(g_sys_param.ssid));
    strlcpy(g_sys_param.password, password ? password : "", sizeof(g_sys_param.password));
    strlcpy(g_sys_param.key, key ? key : "", sizeof(g_sys_param.key));
