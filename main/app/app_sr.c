...
#define AFE_FEED_CHANNEL_NUM 3
#define AFE_MIC_CHANNEL_NUM  2
#define AFE_REF_CHANNEL_NUM  1
...
static void audio_feed_task(void *arg)
{
    ...
    int feed_channel = AFE_FEED_CHANNEL_NUM;
    ...
    int16_t *i2s_buffer = heap_caps_malloc(audio_chunksize * sizeof(int16_t) * I2S_CHANNEL_NUM, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    int16_t *record_buffer = heap_caps_malloc(audio_chunksize * sizeof(int16_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    assert(audio_buffer);
    assert(i2s_buffer);
    ...
    while (true) {
...
#endif
        esp_err_t read_ret = codec_handle->i2s_read_fn((char *)i2s_buffer, audio_chunksize * I2S_CHANNEL_NUM * sizeof(int16_t), &bytes_read, portMAX_DELAY);
        if (read_ret != ESP_OK || bytes_read == 0) {
            ESP_LOGW(TAG, "i2s_read ret=%s bytes=%u", esp_err_to_name(read_ret), (unsigned)bytes_read);
            continue;
        }
        ...
        /* Channel Adjust: feed AFE as mic0, mic1, ref; save mic0 only for STT. */
        for (int i = 0; i < audio_chunksize; i++) {
            int16_t mic0 = i2s_buffer[i * I2S_CHANNEL_NUM];
            int16_t mic1 = i2s_buffer[i * I2S_CHANNEL_NUM + 1];
            audio_buffer[i * AFE_FEED_CHANNEL_NUM + 0] = mic0;
            audio_buffer[i * AFE_FEED_CHANNEL_NUM + 1] = mic1;
            audio_buffer[i * AFE_FEED_CHANNEL_NUM + 2] = 0;
            record_buffer[i] = mic0;
        }
        if (WIFI_STATUS_CONNECTED_OK == wifi_connected_already()) {
            afe_handle->feed(afe_data, audio_buffer);
        }
        audio_record_save(record_buffer, audio_chunksize);
    }
}
...
static void audio_detect_task(void *arg)
{
    ...
    static uint16_t frame_keep = 0;
    static uint16_t listen_frames = 0;
    ...
    while (true) {
        ...
        if (true == detect_flag) {
            ...
            if (((frame_keep >= 100) && (AFE_VAD_SILENCE == res->vad_state)) || (listen_frames >= 300)) {
...
esp_err_t app_sr_set_language(sr_language_t new_lang)
{
    ...
    ESP_LOGI(TAG, "load wakenet:%s", wn_name ? wn_name : "(null)");
...
esp_err_t app_sr_start(bool record_en)
{
    ...
    afe_config.pcm_config.total_ch_num = AFE_FEED_CHANNEL_NUM;
    afe_config.pcm_config.mic_num = AFE_MIC_CHANNEL_NUM;
    afe_config.pcm_config.ref_num = AFE_REF_CHANNEL_NUM;
