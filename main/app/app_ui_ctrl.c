...
#define REPLY_SCROLL_TIMER_INTERVAL_MS  (1500)
...
static void volume_hide_timer_handler(lv_timer_t *timer);
...
void ui_ctrl_init(void)
{
    ...
    /* volume indicator (bar + label), hidden until a button event shows it */
    volume_bar = lv_bar_create(ui_ScreenListen);
    lv_obj_set_size(volume_bar, 160, 10);
    ...
    volume_label = lv_label_create(ui_ScreenListen);
    lv_obj_set_style_text_font(volume_label, &ui_font_PingFangEN16, LV_PART_MAIN);
    lv_obj_set_style_text_color(volume_label, lv_color_hex(0xFFFFFF), LV_PART_MAIN);
    lv_obj_align(volume_label, LV_ALIGN_BOTTOM_MID, 0, -26);
    lv_obj_add_flag(volume_label, LV_OBJ_FLAG_HIDDEN);

    ui_ball_anim_start(UI_CTRL_PANEL_SLEEP);
...
static void reply_content_show_text(const char *text)
{
    ...
    *(decode + j) = '\0';
    ...
    lv_obj_set_align(ui_LabelReplyContent, LV_ALIGN_TOP_LEFT);
    lv_obj_update_layout(ui_LabelReplyContent);
    content_height = lv_obj_get_height(ui_LabelReplyContent) + 16;
    lv_obj_scroll_to_y(ui_ContainerReplyContent, 0, LV_ANIM_OFF);
    reply_content_get = true;
    lv_timer_resume(scroll_timer_handle);
    ESP_LOGI(TAG, "scroll start: content=%d container=%d", content_height, lv_obj_get_height(ui_ContainerReplyContent));
...
void ui_ctrl_volume_show(int volume, bool muted)
{
    bsp_display_lock(0);

    if (volume_bar == NULL || volume_label == NULL) {
        bsp_display_unlock();
        return;
    }

    lv_bar_set_value(volume_bar, muted ? 0 : volume, LV_ANIM_ON);
    if (muted) {
        lv_label_set_text(volume_label, "MUTE");
    } else {
        lv_label_set_text_fmt(volume_label, "VOL %d", volume);
    }
    ...
    volume_hide_timer = lv_timer_create(volume_hide_timer_handler, 1500, NULL);
