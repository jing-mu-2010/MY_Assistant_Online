...
/* Codec function-pointer handle (replaces bsp_board.h) */
typedef struct {
    esp_err_t (*i2s_write_fn)(void *audio_buffer, size_t len, size_t *bytes_written, uint32_t timeout_ms);
    esp_err_t (*i2s_read_fn)(void *audio_buffer, size_t len, size_t *bytes_read, uint32_t timeout_ms);
    esp_err_t (*mute_set_fn)(bool mute);
    esp_err_t (*volume_set_fn)(int volume, void *aux);
    esp_err_t (*i2s_reconfig_clk_fn)(uint32_t sample_rate, uint32_t bits_per_sample, i2s_slot_mode_t slot_mode);
    esp_err_t (*codec_reconfig_fn)(void);
} bsp_codec_config_t;
...
/* Button-facing volume / mute control (ES8311 DAC over BOX1 I2C) */
